...
class GridFSAuthStrategy extends BaseAuthStrategy {
  ...
  async destroy() { await this.saveSession(); if (fs.existsSync(this.userDataDir)) fs.rmSync(this.userDataDir, { recursive: true, force: true }); }
  async deletePermanently() { await this.deleteExistingSession(); if (fs.existsSync(this.userDataDir)) fs.rmSync(this.userDataDir, { recursive: true, force: true }); }
  async sessionExists() { try { const bucket = this.initGridFS(); if (!bucket) return false; const files = await bucket.find({ 'metadata.clientId': this.clientId }).limit(1).toArray(); return files.length > 0; } catch { return false; } }
  async getSessionInfo() { try { const bucket = this.initGridFS(); if (!bucket) return null; const files = await bucket.find({ 'metadata.clientId': this.clientId }).toArray(); if (files.length === 0) return null; const totalSize = files.reduce((s, f) => s + f.length, 0); const lastModified = Math.max(...files.map(f => f.metadata.sessionVersion || 0)); return { clientId: this.clientId, fileCount: files.length, totalSize, lastModified: new Date(lastModified), files: files.map(f => ({ name: f.filename, size: f.length, path: f.metadata.originalPath })) }; } catch { return null; } }
}
...
async function startClient(userId, clientsMap = null, clientStatesMap = null) {
  return _runWithLimit(async () => {
    ...
    client.on('qr', async (qr) => {
      ...
      clientStatesMap?.set(uid, { connected: false, qr: qrBase64, status: 'awaiting_qr', timestamp: new Date().toISOString() });
    });
    ...
    client.on('ready', async () => {
      ...
      clientStatesMap?.set(uid, { connected: true, qr: null, status: 'connected', timestamp: new Date().toISOString() });

      setTimeout(async () => { try { await client.authStrategy.saveSession(); } catch {} }, 10000);
    ...
    client.on('disconnected', async (reason) => {
      ...
      clearInterval(saveInterval);
      try { await client.authStrategy.saveSession(); } catch {}
      clientStatesMap?.set(uid, { connected: false, qr: null, status: 'disconnected', reason, timestamp: new Date().toISOString() });
    });

    client.on('auth_failure', (msg) => {
      console.log(`[${uid}] ❌ Falha de autenticação:`, msg);
      clearInterval(saveInterval);
      clientStatesMap?.set(uid, { connected: false, qr: null, status: 'auth_failed', error: msg, timestamp: new Date().toISOString() });
    });
  ...
  // timeout de 30s para não travar fila
